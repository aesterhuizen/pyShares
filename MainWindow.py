...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui.cmbAction.addItem("sell_total_return_except_x")
        ...
        self.ui.lbltblAsset_sum.setText("Total Equity: $0.00")
        ...
        if os.path.exists(self.env_file):
            ...
            if button == 1:
                ...
                try:
                    self.portfolio = self.get_stocks_from_portfolio(self.current_account_num)  # Sort by the 10th element (index 9)
                except Exception as e:
        ...
        self.ui.edtFileBrowse_Reinvest.textChanged.connect(self.edtFileBrowse_Reinvest_changed)
        self.ui.edtStocksInFile_Reinvest.textChanged.connect(self.edtStocksInFile_Reinvest_changed)
    ...
    def edtAmountEst_changed(self):
        ...
        return
    def edtStocksInFile_Reinvest_changed(self):
        if len(self.get_tickers_from_selected_lstAssets()) > 0:
            self.ui.edtFileBrowse_Reinvest.setEnabled(False)
            self.ui.btnBrowseReInvest.setEnabled(False)       
        else:
            self.ui.edtFileBrowse_Reinvest.setEnabled(True)
            self.ui.btnBrowseReInvest.setEnabled(True)
        return
    def edtFileBrowse_Reinvest_changed(self):
        if os.path.isfile(self.ui.edtFileBrowse_Reinvest.text()):
            self.ui.edtReinvestAmount.setEnabled(False)
            self.ui.edtStocksInFile_Reinvest.setEnabled(False)
    ...
    def cmbReInvest_clicked(self):
        ...
        if os.path.isfile(self.ui.edtFileBrowse_Reinvest.text()):
            ...
            if self.ui.edtReinvestAmount.text() != '':
                try:
                    total_amount = float(self.ui.edtReinvestAmount.text())
    ...
    def btnBrowseReInvest_clicked(self):
        ...
        if os.path.isfile(file_name):
            self.ui.edtFileBrowse_Reinvest.setText(file_name)
            self.ui.edtReinvestAmount.setEnabled(False)
            self.ui.edtStocksInFile_Reinvest.setEnabled(False)
            ...
            try:
                ...
                self.ui.edtFileBrowse_Reinvest.setText("")
                self.ui.edtStocksInFile_Reinvest.setReadOnly(False)
                self.ui.edtStocksInFile_Reinvest.setStyleSheet("background-color: white;")
                self.ui.edtStocksInFile_Reinvest.setText("")
    ...
    def btnBrowseBuySell_clicked(self):
        ...
        if os.path.isfile(file_name):
            ...
            self.ui.edtLstStocksSelected.setReadOnly(True)
            self.ui.edtLstStocksSelected.setStyleSheet("background-color: lightgrey;")
            try:
                with open(file_name,"r") as open_file:
                    lines = open_file.readlines()
                    for line in lines:
                ...
                self.ui.edtLstStocksSelected.setReadOnly(False)
                self.ui.edtLstStocksSelected.setStyleSheet("background-color: white;")
                self.ui.edtLstStocksSelected.setText("")
                self.ui.edtAmountEst.setText("")
                return
            ...
            if self.ui.cmbAction.currentText() == "sell_total_return_except_x" :
                ...
                lst_share_name = self.find_and_remove(self.portfolio, stocks,0)
                # FIX ME total_amount = sum(item for item in lst_share_name)
            ...
            self.ui.edtLstStocksSelected.setText(str_stock_names)
            self.ui.btnExecute.setEnabled(True)
            self.ui.btnExecute.setStyleSheet("background-color: green; color: white;")  # Change background to green             
        else:
            self.ui.edtFileBrowse.setText("")
                ...
                if event.button() == Qt.MouseButton.LeftButton:
                    if self.tooltip:
                        ...
                        if self.ui.cmbAction.currentText() == "sell_selected_total_return" :
                            ...
                            self.ui.edtAmountEst.setText(f"${self.cur_today_return:,.2f}")
    ...
    def updateStatusBar(self, selected_stocks={}) -> tuple[float, float]:
    ...
    def updateLstAssets(self):        
        ...
        if len(self.portfolio) != 0: 
            ...
            self.portfolio = self.update_stocks_dict()

            
           
            
          
            for idx,sym in enumerate(self.portfolio):
                if self._shutting_down:
                    return
                
                lst_elements_to_update.append([self.portfolio[sym]['stock_name'], \
                                                   self.portfolio[sym]['price'], \
                                                   self.portfolio[sym]['change'], \
                                                   self.portfolio[sym]['quantity'], \
                                                   self.portfolio[sym]['total_return'], \
                                                   self.portfolio[sym]['todays_return'], \
                                                   self.portfolio[sym]['equity'], \
    ...
    def tblAsset_clicked(self):
        ...
        if self.ui.cmbAction.currentText() == "raise_x_sell_y_dollars_except_z":
            self.ui.edtRaiseAmount.setReadOnly(True)
        else:
            self.ui.edtAmountEst.setReadOnly(False)
        ...
        if len(stock_names) >0: 
            ...
            lblAsset_totEquity = f"Total Equity: ${total_equity:,.2f}"
            self.ui.lbltblAsset_sum.setText(lblAsset_totEquity)
           
            self.ui.btnExecute.setEnabled(True)
            self.ui.btnExecute.setStyleSheet("background-color: green; color: white;")  # Change background to grey             

            match self.ui.cmbAction.currentText():
                case "stock_info":
                    # ensure not too much data to plot
                    if not len(stock_names) > 1:
                        self.setup_plot(self.portfolio,stock_names,plot_type=self.current_plot_type)
                ...
                case "reinvest_with_gains":
                    if os.path.isfile(self.ui.edtFileBrowse_Reinvest.text()):
                        ...
                        strjoinlst = ",".join(stock_names)
                        self.ui.edtStocksInFile_Reinvest.setText(strjoinlst)
    ...
    def recalculate_estimated_amount(self):
        ...
        if re.match(r'^\d+$',self.ui.edtDollarValueToSell.text()):
            ...
            ticker_copy_include_list = self.find_and_remove(self.portfolio,lstShares,1)
            ...
            if dollar_share == "Buy in USD":
                ...
                for item in ticker_copy_include_list:
                        ...
                        est_price = self.portfolio[item]['price']
                        Total_quantity = float(self.ui.edtDollarValueToSell.text()) / float(est_price)
                        priceTotal += float(est_price) * Total_quantity
    ...
    def clear_all_clicked(self):
        ...
        self.ui.edtReInvestValue.setText("")
        self.ui.edtFileBrowse_Reinvest.setText("")
        self.ui.edtReinvestAmount.setText("")
        #---------------------------------
    ...
    def btnExecute_clicked(self):
        if self.ui.btnExecute.text() == "Cancel":
            ...
            match self.ui.cmbAction.currentText():
                ...
                case "sell_total_return_except_x":
                    self.Execute_operation("sell_total_return_except_x",num_iter,lst)
    ...
    def cmbAction_clicked(self):
        ...
        #page 1 - sell except x ------------------------------------
        self.ui.lblExceptStocks_in_file.setVisible(True)
        self.ui.edtFileBrowse.setVisible(True)
        self.ui.btnBrowse.setVisible(True)
        #------------------------------------------------------------
        self.ui.edtRaiseAmount.setVisible(False)
        self.ui.lblRaiseAmount.setVisible(False)
    ...
    def Execute_operation(self,method_name = None,num_iter = 1,lst=None, ask_confirm=True) -> list:
        ...
        next_oper_lst = [] #list of the next quequed operation
        ...
        if ask_confirm:
            ...
            match method_name:
                ...
                case "sell_total_return_except_x":
                    ...
                    msg =   f"Are you sure you want to execute operation '{self.ui.cmbAction.currentText()}'\n" \
                            ...
                            f"\nPreview:"
                    sell_list = self.sell_total_return_except_x_prev(num_iter,lst,raise_amount,dollar_value_to_sell,buying_with_amount)
                    preview_sellbuy_list = [f"{item[0]}: sell {item[1]:.2f} shares" for item in sell_list]
                    lst = sell_list
                case "sell_todays_return_except_x":
                    amount = float(self.ui.edtAmountEst.text().replace("$","").replace(",",""))
                ...
                case "reinvest_with_gains":
                    ...
                    preview_sellbuy_list = [f"{item[0]}: buy {item[1]:.2f} shares" for item in buy_list]
                    lst = buy_list
                case "allocate_reallocate_to_sectors":
                    pass
                case "raise_x_sell_y_dollars_except_z":
                    ...
                    if self.ui.cmbAction.currentText() == "allocate_reallocate_to_sectors": 
                        sell_list,self.raised_amount = self.raise_x_sell_y_dollars_except_z_prev(num_iter,lst,raise_amount,dollar_value_to_sell,buying_with_amount)
                        buy_list = self.buy_selected_with_x_prev(num_iter,self.symbols_to,raise_amount,dollar_value_to_sell,buying_with_amount)
                        if sell_list or buy_list: 
                            format_sell_list = [f"{item[0]}: sell {float(item[1]):.2f} shares" for item in sell_list]
                            format_buy_list = [f"{item[0]}: buy {float(item[1]):.2f} shares" for item in buy_list]
                        ...
                        sell_list,self.raised_amount = self.raise_x_sell_y_dollars_except_z_prev(num_iter,lst,raise_amount,dollar_value_to_sell,buying_with_amount)
                        if sell_list: 
                            format_sell_list = [f"{item[0]}: sell {item[1]:.2f} shares" for item in sell_list if float(item[1]) != 0.0]
                            preview_sellbuy_list = format_sell_list
                            name_lst = self.ui.edtRaiseAmount.text()
                            if float(self.raised_amount) < float(raise_amount):
                                msg =  f"Are you sure you want to execute operation '{self.ui.cmbAction.currentText()}'\n" \
                                        f"Except: {name_lst}\n" \
                                        f"Raise Amount: Can only raise ${self.raised_amount} when selling ${dollar_value_to_sell} of each share, reduce dollar amount to raise more money\n" \
                                        f"\nPreview:\n"
                            else:
                                msg =   f"Are you sure you want to execute operation '{self.ui.cmbAction.currentText()}'\n" \
                                        f"Except: {name_lst}\n" \
                                        f"Raise Amount: {raise_amount}\n" \
                                        f"\nPreview:\n"
            ...
            #this is a sequence operation, just send the list to the command thread and the rest of the variables is nothing ''
            raise_amount = ""
            dollar_value_to_sell = ""
            buying_with_amount = ""
    ...
    def print_cur_protfolio(self, curlist):
        ...
        self.ui.tblAssets.setHorizontalHeaderLabels(["Ticker","Price","Change","Quantity","Total Return","Today's Return","Equity", "% of Portfolio"])
    ...
    def update_stocks_dict(self):
        ...
        updated_holdings = {}
        for  sym in portfolio_copy:
            qty = portfolio_copy[sym]['quantity']
            equity = float(r.stocks.get_quotes(sym,"last_trade_price")[0]) * qty
            last_price = equity / qty
            previous_close = float(r.stocks.get_quotes(sym, "previous_close")[0])
            updated_holdings[sym] = {
                "price": last_price,
                "change": last_price - previous_close,
                "quantity": qty,
                "equity": equity,
                "todays_return": (last_price - previous_close)*qty,
                "total_return": (last_price - portfolio_copy[sym]["average_buy_price"])*qty,
                "average_buy_price": portfolio_copy[sym]["average_buy_price"],
                "percentage": 0.0,  # filled after total equity known
                "prev_close": previous_close,
                "sector":  portfolio_copy[sym]["sector"],
                "stock_name": portfolio_copy[sym]['stock_name']
                
            } 
        updated_portfolio_tvalue = sum((float(updated_holdings[item]['equity']) for item in updated_holdings))
    ...
    def get_stocks_from_portfolio(self, acc_num):
        ...
        sector = [r.get_fundamentals(ticker, info='sector')[0] for ticker in tickers]
        ...
        for i, sym in enumerate(tickers):
            ...
            holdings[sym] = {
                ...
                "change": float(lastPrice[i]) - float(previous_close[i]),
                "quantity": qty,
                "equity": equity,
                "todays_return": (float(lastPrice[i]) - float(previous_close[i]))*qty,
                "total_return": (float(lastPrice[i]) - avg_buy_price[i])*qty,
                "average_buy_price": float(avg_buy_price[i]),
                "percentage": 0.0,  # filled after total equity known
                "prev_close": float(previous_close[i]),
                "sector":  sector[i],
                "stock_name": f"{stock_name[i]} ({sym})"
        ...
        sorted_holdings = dict(sorted(holdings.items(), key=lambda kv: kv[0]))
    ...
    def find_and_remove(self, tickers, in_exList, include_flag=1):
        ...
        if include_flag == 1:
            updated_dict = {k: v for k, v in self.portfolio.items() if k in in_exList}
        elif include_flag == 0:
            updated_dict = {k: v for k, v in self.portfolio.items() if k not in in_exList}
        ...
        match self.ui.cmbAction.currentText() :
            ...
            case "sell_todays_return_except_x" | "sell_total_return_except_x": 
                ...
                if lst:
                    ...
                    msg = QMessageBox.warning(self,"Selection","No stocks selected from the asset list.",QMessageBox.StandardButton.Ok)
                    if msg == QMessageBox.StandardButton.Ok:
                        lst = ['err']
                
            case "raise_x_sell_y_dollars_except_z":
                lst = self.ui.edtRaiseAmount.text().split(",")
                dict_slice = self.find_and_remove(self.portfolio.items,lst,0)
                lst = [item for item in dict_slice.keys()]
            case "reinvest_with_gains":
                lst = [item for item in self.ui.edtStocksInFile_Reinvest.text().split(",")]
                if lst:
                    check_two = True
                else:
                    msg = QMessageBox.warning(self,"Selection","No stocks selected from the asset list.",QMessageBox.StandardButton.Ok)
    ...
    def reinvest_with_gains_prev(self,num,lst,raise_amount,dollar_value_to_sell,buying_with_amount):
        ...
        for stock in lst:
            ...
            if stock_market_prices is None:
                ...
                buy_list.append( [stock,per_quantity,buy_price] )
                spent += per_quantity*buy_price
...
# -------------------------------------------------------------------------------------------------------------------------------------
    def buy_selected_with_x_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        quantity_to_buy = 0.0
            ...
            #include the tickets in lst, exclude other tickers
        dict_slice = self.find_and_remove(self.portfolio, lst,1)

       
        if dict_slice is None or len(dict_slice) == 0:
            ...
            if buying_power != 0.0: # user entered a buying power amount
                ...
                while not (buying_power <= 0):
                    for item in dict_slice.items():
                        ...
                        latest_price = self.portfolio[item[0]]['price']#r.stocks.get_latest_price(item[0])[0]
                        if latest_price is None:
                            ...
                            if self.ui.cmbDollarShare.currentText() == "Buy in Shares":
                                ...
                                quantity_to_buy = dollar_value_to_buy / float(latest_price)
                    
                        tot = quantity_to_buy*float(latest_price)
                        ...
                        for i,value  in enumerate(buy_list): #check if already in list and return index
                            if value[0] == item[0]:
                                ...
                                if tot < buying_power:    
                                    buy_list[i][1] = float(exist_quantity) + quantity_to_buy
                                    buying_power -= tot
                                    break
                            else:
                                found = False
                         ...
                         # item[2] = latest price               
                        if not found:
                            itm = [item[0],quantity_to_buy,latest_price]
                            buy_list.append(itm)
                            buying_power -= tot 
            else:
                    #only selected lst of stocks to buy
                    for item in dict_slice.items():
    ...
    def buy_selected_with_x(self,n,acc_num,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        for n in range(int(n)):
            ...
            for item in lst:    
                ...
                if os.environ['debug'] == '0':
                    try:
                        buy_info = r.order(symbol=item[0],quantity=frm_quantity,side='buy',timeInForce='gfd',limitPrice=item[2]+0.01,account_number=acc_num)
                ...
                self.lstTerm_update_progress_fn(f"{frm_quantity} of {item[0]} shares bought at market price - ${fill_price} - Total: ${frm_total}")
                time.sleep(2)
...
# ---------------------------------------------------------------------------------------------------------------------------------    
    def sell_selected_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        sell_lst = []
        for item in lst:
            stock_market_prices = self.portfolio[item]['price']#r.stocks.get_latest_price(item)[0]
            if stock_market_prices is None:
                quantity_to_sell = 0.0
            else:
               quantity_to_sell = float(dollar_value_to_sell) / stock_market_prices
            
            sell_lst.append( [item,quantity_to_sell,stock_market_prices] )
...
# -------------------------------------------------------------------------------------------------------------------------------------        
    def sell_total_return_except_x_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
       ...
       #-------------------------------
        ...
        slice_dict = self.find_and_remove(self.portfolio, lst,0)
                
                #exclude the tickets in excludeList
        
        for item in slice_dict.items():
            last_price = self.portfolio[item[0]]['price']
            if last_price is None:
                next
            else:    
                quantity_to_sell = self.portfolio[item[0]]['total_return'] / float(last_price)
                if quantity_to_sell > 0.0:
                    sell_list.append( [item[0],quantity_to_sell,float(last_price)] )
...
# sell_gains_x exclude a list of stocks
# -------------------------------------------------------------------------------------------------------------------------------------        
    def sell_total_return_except_x(self,n,acc_num,lst,raise_amount,dollar_value_to_sell,buying_with):
        #lst -------------------------
        #Item 0 =  tickers
        #Item 1 = quantity to sell
        #Item 2 = last price
        ...
        for index in range(int(n)):
            ...
            for item in lst: #
                ...
                if self.portfolio[item[0]]["quantity"] - item[1] >= 0.1:
                    if os.environ['debug'] == '0':
                        try:
                            sell_info = r.order_sell_market(symbol=item[0],quantity=frm_quantity,timeInForce='gfd',account_number=acc_num)
                            time.sleep(0.5)
                            sell_info = r.get_stock_order_info(sell_info['id'])
                            #if not state infor is filled then wait till it is filled
                            while 'state' not in sell_info and sell_info['state'] != 'filled' and sell_info['state'] != 'queued':
    ...
    def sell_selected_todays_return_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        for item in lst:
            ...
            if last_price is None:
                ...
                quantity_to_sell = self.portfolio[item]['todays_return'] / float(last_price)
                if quantity_to_sell > 0.0:
                    sell_list.append( [item,quantity_to_sell,last_price] )
            
         
        return sell_list
    def sell_selected_total_return_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        for item in lst:
            ...
            if last_price is None:
                ...
                quantity_to_sell = self.portfolio[item]['total_return'] / float(last_price)
                if quantity_to_sell > 0.0:
                    sell_list.append( [item,quantity_to_sell,float(last_price)] )
    ...
    def sell_selected_total_return(self,n,acc_num,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        for index in range(int(n)):
            ...
            for item in lst:
                ...
                frm_shares_to_sell = "{0:.2f}".format(shares_to_sell)
                #if share quantity > shares to sell then sell shares_to_sell
                if self.portfolio[item[0]]["quantity"] - shares_to_sell >= 0.1:
                    if os.environ['debug'] == '0':
                        try:
                            sell_info = r.order_sell_market(symbol=item[0],quantity=frm_shares_to_sell,timeInForce='gfd',account_number=acc_num)
                            time.sleep(0.5)
    ...
    def sell_selected_todays_return(self,n,acc_num,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        return
#----------------------------------------------------------------------------------------------------------------------------------
# sell__todays_return preview
# -------------------------------------------------------------------------------------------------------------------------------------        
    def sell_todays_return_except_x_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
       ...
       #-------------------------------
        ...
        for item in slice_dict.items():
            ...
            if last_price is None:
                ...
                quantity_to_sell = self.portfolio[item[0]]['todays_return'] / float(last_price)
                if quantity_to_sell > 0.0:
                    sell_list.append( [item[0],quantity_to_sell,float(last_price)] )
...
#---------------------------------------------------------------------------------------------------------------------------------
    def raise_x_sell_y_dollars_except_z_prev(self,n,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        n_raise_amount = float(raise_amount) if raise_amount != "" else 0.0
        ...
        while not (raised_amount >= n_raise_amount or raise_amount_converged==True):
            ...
            for item in lst:
                ...
                latest_price = self.portfolio[item]['price']#r.stocks.get_latest_price(item[0])[0]
                
                if latest_price is None:
                    quantity_to_sell = 0.0
                    latest_price = 0.0
                else:  
                    quantity_to_sell = n_dollar_value_to_sell /  float(latest_price)

                if raised_amount >= n_raise_amount or raise_amount_converged:
                    break    

                if not (self.portfolio[item]['quantity'] > quantity_to_sell+0.1): #check if you have enough shares to sell
                    quantity_to_sell = 0.0    
                ...
                for i,value  in enumerate(sell_list): #check if already in list and return index
                    ...
                    if (value[0] == item) and (quantity_to_sell != 0.0):
                        exist_quantity = sell_list[i][1]
                        found = True
                        # stock_quantity_to_sell and (your quantity > stock_quantity_to_sell)
                        total_new_quantity = float(exist_quantity) + quantity_to_sell
                        
                        if (total_new_quantity <= self.portfolio[item]['quantity'] + 0.1):
                            sell_list[i][1] = total_new_quantity
                            raised_amount += quantity_to_sell*float(latest_price)
                ...
                if not found:
                   #don't add to list when you do not have enough shares to sell
                    if  total_new_quantity+0.1 <= self.portfolio[item]['quantity']:
                        itm = [item,quantity_to_sell,float(latest_price)]
...
#---------------------------------------------------------------------------------------------------------------------------------
    def raise_x_sell_y_dollars_except_z(self,n,acc_num,lst,raise_amount,dollar_value_to_sell,buying_with):
        ...
        if method_name == "allocate_reallocate_to_sectors":
            self.lstTerm_update_progress_fn(f"Raise Amount: ${frmt_raise_amount} by selling {[item[0] for item in lst]}")
        else:
            self.lstTerm_update_progress_fn(f"Raise Amount: ${frmt_raise_amount} by selling except {[item[0] for item in lst]}")
    ...
    #     #place sell orders
        for item in sell_lst:        
            ...
            self.lstTerm_update_progress_fn(f"{frm_quantity_to_sell:.2f} shares of {item[0]} sold at market price ${fill_price} - Total: ${frm_tot} ")                    
            time.sleep(5)
